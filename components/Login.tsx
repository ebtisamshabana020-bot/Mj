...
import { getProfile, signInWithPassword, upsertProfile } from './services/firebaseClient';
...
const Login: React.FC<LoginProps> = ({ onLogin, onGoToRegister }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!auth.localId) throw new Error('بيانات الدخول غير صحيحة.');

      let profile = await getProfile(auth.localId, auth.idToken);
      if (!profile) {
    ...
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl w-full max-w-md border border-slate-100">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 mt-2 font-medium">حسابك متصل بسيرفر Firebase</p>
        </div>
