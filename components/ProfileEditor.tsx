...
import { getIdToken, upsertProfile } from './services/firebaseClient';
...
const ProfileEditor: React.FC<ProfileEditorProps> = ({ user, onUpdate, onBack }) => {
  ...
  const saveProfile = async () => {
    ...
    setSaving(true);
    try {
      await upsertProfile(user.id, { username: username.trim(), avatar_url: avatar ?? null }, getIdToken() ?? undefined);
      onUpdate({ ...user, username: username.trim(), avatar });
      onBack();
    } catch (error: any) {
      alert(error?.message || 'تعذر تحديث الملف الشخصي.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-2xl mx-auto space-y-6 text-right" dir="rtl">
      ...
      <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 space-y-6">
        <div className="flex justify-center">
          <label className="cursor-pointer">
            <div className="w-24 h-24 rounded-3xl bg-slate-100 overflow-hidden border-2 border-dashed border-slate-300">
              {avatar ? <img src={avatar} alt="avatar" className="w-full h-full object-cover" /> : <div className="h-full flex items-center justify-center text-2xl text-slate-400">+</div>}
            </div>
        ...
        <div>
          ...
          <input value={username} onChange={(event) => setUsername(event.target.value)} className="w-full bg-slate-50 p-4 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500" />
        </div>
